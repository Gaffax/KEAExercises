...
public class ProcessAlice {
	...
	public void showLetterPercentageDistribution() {
		...
		DecimalFormat decimalFormat = new DecimalFormat("0.##");	//Limits digits
	...
	private void analyseText(File alice) throws FileNotFoundException {
		...
		while (input.hasNext()) {
			anArray = input.next().toLowerCase().replaceAll("[^a-z]", "") // Everything is set to lower case, and we remove symbols.
					.toCharArray();										  // I am aware, that toLowerCase and replace all is called
																		  // The individual word, and not the entire document. Which
			for (char letter : anArray) {								  // for most words aren't necessary. This part of the code
				int currentNumber = (int) numberOfLetters.get(letter);	  // Could be rewritten to optimize speed and resources used.
